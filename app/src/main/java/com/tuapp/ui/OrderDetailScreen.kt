...
fun OrderDetailScreen(orderId: String) {
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("OT #$id") }) }) { p ->
        Column(Modifier.padding(p).padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            ...
            ModelDropdown(brand = order?.vehicle?.brand, currentModel = order?.vehicle?.model) { selectedModel ->
                repo.updateVehicle(id, model = selectedModel, year = null, engineCode = null)
                order = repo.getOrder(id)
            }

            YearDropdown(brand = order?.vehicle?.brand, model = order?.vehicle?.model, currentYear = order?.vehicle?.year) { selectedYear ->
            ...
            LazyRow(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                items(o.photos) { ph ->
                    ...
                    if (bm != null) {
                        Image(bm.asImageBitmap(), contentDescription = "${ph.stage}", modifier = Modifier.size(120.dp))
                    }
                }
            }
